...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an expanded landmark-window ASL classifier.")
    ...
    parser.add_argument(
        "--words-labels-csv",
        type=Path,
        default=PROJECT_ROOT / "words-data" / "labels.csv",
        help="Word dataset labels.csv. Defaults to words-data/labels.csv.",
    )
    parser.add_argument(
        "--sequence-labels-csv",
        type=Path,
...
def load_checkpoint(
    ...
    if checkpoint_input_dim != INPUT_FEATURES_PER_FRAME:
        raise RuntimeError(
            ...
            f"but this expanded classifier uses {INPUT_FEATURES_PER_FRAME}. "
    ...
    if checkpoint_labels is not None and tuple(checkpoint_labels) != OUTPUT_LABELS:
        raise RuntimeError(
            "Checkpoint was trained with a different output vocabulary. "
            "Run with --reset or pass a new --checkpoint path."
        )

    checkpoint_state = payload["model_state_dict"]
    classifier_weight = checkpoint_state.get("classifier.2.weight")
    if classifier_weight is not None and classifier_weight.size(0) != len(OUTPUT_LABELS):
        raise RuntimeError(
            f"Checkpoint output head has {classifier_weight.size(0)} classes, "
            f"but the current vocabulary has {len(OUTPUT_LABELS)} labels. "
            "Run with --reset or pass a new --checkpoint path."
        )
...
def main() -> None:
    ...
    if args.checkpoint is None:
        ...
        checkpoint_name = (
            "extra_words_left_hand_torso_face_sequences.pt"
            if uses_extra_manifest
            else "extra_words_left_hand_torso_face_letters.pt"
        )
        args.checkpoint = PROJECT_ROOT / "models" / checkpoint_name

    torch.manual_seed(args.seed)
    device = resolve_device(args.device)
    print(f"Output vocabulary: {len(OUTPUT_LABELS)} labels ({', '.join(OUTPUT_LABELS)})")
    ...
    if args.words_labels_csv is not None:
        if args.words_labels_csv.exists():
            word_examples = read_single_letter_label_examples(
                args.words_labels_csv,
                window_frames=args.window_frames,
                stride_frames=args.single_letter_window_stride,
            )
            print(f"Loaded {len(word_examples)} word examples from {args.words_labels_csv}")
            examples.extend(word_examples)
        else:
            print(f"Skipped missing words-data labels CSV: {args.words_labels_csv}")
