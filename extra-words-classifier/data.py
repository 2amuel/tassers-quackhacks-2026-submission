...
from model import LETTERS, OUTPUT_LABELS, WORDS
...
POSE_FEATURES_PER_FRAME = predictor.model_module.POSE_FEATURES
FACE_FEATURES_PER_FRAME = predictor.model_module.FACE_FEATURES
LEFT_HAND_START = 0
LEFT_HAND_END = SINGLE_HAND_FEATURES_PER_FRAME
POSE_START = ALL_HAND_FEATURES_PER_FRAME
FACE_START = POSE_START + POSE_FEATURES_PER_FRAME
TORSO_POSE_LANDMARKS = (11, 12, 13, 14, 23, 24)
FACE_POSITION_LANDMARKS = (1, 10, 152, 234, 454)
FACE_LANDMARK_TO_COMPACT_INDEX = {
    landmark: index for index, landmark in enumerate(predictor.SELECTED_FACE_LANDMARKS)
}
TORSO_FEATURES_PER_FRAME = len(TORSO_POSE_LANDMARKS) * FEATURES_PER_LANDMARK
FACE_POSITION_FEATURES_PER_FRAME = len(FACE_POSITION_LANDMARKS) * FEATURES_PER_LANDMARK
INPUT_FEATURES_PER_FRAME = (
    SINGLE_HAND_FEATURES_PER_FRAME + TORSO_FEATURES_PER_FRAME + FACE_POSITION_FEATURES_PER_FRAME
)
LABEL_TO_CLASS = {label: index for index, label in enumerate(OUTPUT_LABELS)}
WORD_LABELS = set(WORDS)
WRIST_INDEX = 0
MIDDLE_MCP_INDEX = 9
FEATURE_NORMALIZATION = "left_hand_shape_torso_face_position_v1"
...
def discover_isolated_letter_examples(letter_landmarks_dir: Path) -> list[LetterExample]:
    ...
    for folder in sorted(path for path in letter_landmarks_dir.iterdir() if path.is_dir()):
        ...
        if letter in LABEL_TO_CLASS and csv_path.exists():
...
def read_sequence_label_examples(labels_csv: Path) -> list[LetterExample]:
    ...
    with labels_csv.open(newline="", encoding="utf-8") as labels_file:
        ...
        for row in reader:
            ...
            if should_use_whole_label(expected_text, row):
                examples.append(
                    LetterExample(
                        clip_id=clip_id,
                        csv_path=csv_path,
            ...
            if any(letter not in LABEL_TO_CLASS for letter in expected_text):
                print(
                    f"Skipping {row.get('clip_id', '<unknown>')}: "
                    f"unsupported label {expected_text!r}"
                )
                continue
...
def should_use_whole_label(expected_text: str, row: dict[str, str]) -> bool:
    if expected_text not in LABEL_TO_CLASS:
        return False
    if len(expected_text) == 1:
        return True
    notes = row.get("notes", "").lower()
    return expected_text in WORD_LABELS and "word" in notes
    ...
    with labels_csv.open(newline="", encoding="utf-8") as labels_file:
        ...
        for row in reader:
            ...
            if expected_text not in LABEL_TO_CLASS:
                print(
                    f"Skipping {row.get('clip_id', '<unknown>')}: "
                    f"expected A-Z or supported word label, got {expected_text!r}"
                )
                continue
...
def single_letter_windows(
    ...
    if num_frames <= window_frames:
        return [LetterExample(clip_id=clip_id, csv_path=csv_path, label=label)]
...
def landmark_features(
    ...
    for landmark_index in landmark_indices:
        feature_start = start + landmark_index * FEATURES_PER_LANDMARK
        feature_end = feature_start + FEATURES_PER_LANDMARK
        chunks.append(features[:, feature_start:feature_end])
...
def torso_features(features: torch.Tensor) -> torch.Tensor:
    return landmark_features(features, POSE_START, TORSO_POSE_LANDMARKS)


def face_position_features(features: torch.Tensor) -> torch.Tensor:
    compact_indices = tuple(
        FACE_LANDMARK_TO_COMPACT_INDEX[landmark]
        for landmark in FACE_POSITION_LANDMARKS
        if landmark in FACE_LANDMARK_TO_COMPACT_INDEX
    )
    return landmark_features(features, FACE_START, compact_indices)
...
def normalize_hand_shape_features(features: torch.Tensor) -> torch.Tensor:
    ...
    shaped[:, :, :3] = coords
    return shaped.reshape(features.size(0), SINGLE_HAND_FEATURES_PER_FRAME)


def model_input_features(features: torch.Tensor) -> torch.Tensor:
    return torch.cat(
        (
            normalize_hand_shape_features(left_hand_features(features)),
            torso_features(features),
            face_position_features(features),
        ),
        dim=1,
    )
...
def csv_to_feature_tensor(csv_path: Path, normalize_to_body: bool) -> torch.Tensor:
    ...
    return model_input_features(features)
...
class LetterWindowDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        return {
            ...
            "features": window,
            "target": torch.tensor(LABEL_TO_CLASS[example.label], dtype=torch.long),
            "label": example.label,
        }
